# -*- coding: utf-8 -*-
"""Hostile AST Hardening, Prohibited Primitives, and Zero-Actuation Audit for M13."""
...
def test_ast_prohibited_imports_audit() -> None:
    """Verify zero prohibited external imports in python/holomed/registration/."""
    reg_dir = Path(__file__).resolve().parent.parent.parent.parent / "python" / "holomed" / "registration"
    py_files = list(reg_dir.glob("*.py"))
...
def test_ast_dangerous_primitives_audit() -> None:
    """Verify zero dangerous primitives (eval, exec, etc.) in python/holomed/registration/."""
    reg_dir = Path(__file__).resolve().parent.parent.parent.parent / "python" / "holomed" / "registration"
    for path in reg_dir.glob("*.py"):
        tree = ast.parse(path.read_text(encoding="utf-8"))
        for node in ast.walk(tree):
