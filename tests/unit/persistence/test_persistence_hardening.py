# -*- coding: utf-8 -*-
"""Hardening and AST Security Audit Tests for M09 Persistence Subsystem."""
...
def test_ast_prohibited_imports_and_calls() -> None:
    """Verify persistence source contains zero prohibited modules or dangerous execution calls."""
    target_dirs = [
        Path("python/holomed/persistence"),
...
def test_no_private_platform_attribute_access() -> None:
    """Verify persistence source does not access private platform attributes on external objects."""
    source_dir = Path("python/holomed/persistence")
    prohibited_private_attrs = {"_sessions", "_cycle_history", "_observations", "_context_history"}
    ...
    for py_file in source_dir.glob("*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Attribute):
                # Only check access on non-self objects
