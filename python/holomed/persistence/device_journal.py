# -*- coding: utf-8 -*-
"""Append-Only JSONL Device Journal with Hash Chaining and Crash Recovery."""
...
from holomed.devices.models import DEVICE_ID_REGEX

GENESIS_PREVIOUS_HASH: str = "0" * 64

META_ENTRY_TYPES: tuple[DeviceJournalEntryType, ...] = (
    DeviceJournalEntryType.DEVICE_QUARANTINED,
    DeviceJournalEntryType.ISOLATION_TRANSACTION_INTENTED,
    DeviceJournalEntryType.ISOLATION_PARTICIPANTS_PREPARED,
...
def validate_device_path(storage_root: Path, device_id: str) -> Path:
    """Resolve and validate journal path preventing directory traversal attacks."""
    if type(device_id) is not str or not DEVICE_ID_REGEX.match(device_id):
        raise PersistenceSecurityError(f"Invalid characters or format in device_id: {device_id!r}")

    root = storage_root.resolve()
    target = (root / f"{device_id}.jsonl").resolve()
...
class DeviceJournalWriter:
    """Manages append-only JSONL device journal writing with cryptographic hash chaining."""
    ...
    def __init__(
        ...
        self._journal_path = validate_device_path(self._storage_root, device_id)
    ...
    def append_entry(
        ...
        if self._is_closed:
            raise PersistenceLifecycleError(
                f"DeviceJournalWriter for device {self._device_id} is closed"
            )
        ...
        try:
            if self._get_authoritative_epoch is not None:
                ...
                if self._epoch_id != current_epoch:
                    raise PersistenceEpochMismatchError(
                        f"Stale DeviceJournalWriter (epoch {self._epoch_id}) rejected; authoritative epoch is {current_epoch}"
                    )
            ...
            with open(self._journal_path, "a+b") as f:
                ...
                try:
                    ...
                    raise PersistenceLifecycleError(
                        f"Concurrent write rejected: failed to acquire exclusive lock on device journal {self._device_id}"
                    ) from e

                try:
                    recovered_entries, truncated = DeviceJournalReader.read_and_recover_journal(self._journal_path, file_obj=f)
                    ...
                    if resolved_sequence <= actual_last_seq and entry_type not in META_ENTRY_TYPES:
                        raise PersistenceSequenceError(
                            f"Non-monotonic sequence number {resolved_sequence} <= {actual_last_seq} for device {self._device_id}"
                        )

                    if actual_count >= MAX_JOURNAL_ENTRIES_PER_SESSION:
                        raise PersistenceCapacityError(
                            f"Journal entry count exceeded {MAX_JOURNAL_ENTRIES_PER_SESSION} for device {self._device_id}"
                        )
...
class DeviceJournalReader:
    """Reads and validates append-only device journal files with crash recovery."""

    @staticmethod
    def read_and_recover_journal(
        journal_path: Path,
        file_obj: Any = None,
    ) -> tuple[list[DeviceJournalEntry], int]:
        ...
        entries: list[DeviceJournalEntry] = []
        ...
        for i, line in enumerate(lines):
            ...
            try:
                ...
                raise PersistenceCorruptionError(
                    f"Malformed JSON at record index {i} in device journal {journal_path.name}: {e}"
                ) from e
            ...
            entry = DeviceJournalEntry(
                ...
                entry_type=DeviceJournalEntryType(record["entry_type"]),
