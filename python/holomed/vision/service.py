"""Master spatial vision domain service implementing IService for M01 with M39 Session Isolation."""
...
class VisionService(IService):
    """Spatial Vision & Tracking Service implementing IService with Session Isolation."""
    ...
    def __init__(
        ...
        # Session-Partitioned Subsystems
    ...
    def resources(self) -> OwnedResourceSet:
        if self._resources is None:
            raise VisionLifecycleError(
                "Service has not been initialized; resources unavailable"
            )
        return self._resources

    @property
    def frame_store(self) -> Optional[VisionFrameStore]:
        if self._session_stores:
            return next(iter(self._session_stores.values()))
        return None

    @property
    def tracker(self) -> Optional[TemporalLandmarkTracker]:
        if self._session_trackers:
            return next(iter(self._session_trackers.values()))
        return None

    @property
    def pipeline(self) -> Optional[VisionPipeline]:
        if self._session_pipelines:
            return next(iter(self._session_pipelines.values()))
        return None
    ...
    def initialize(self, context: RuntimeContext) -> None:
        ...
        if self._state != ServiceState.UNINITIALIZED:
            raise VisionLifecycleError(
                f"Cannot initialize VisionService in state {self._state.name}"
            )
    ...
    def start(self) -> None:
        ...
        if self._state != ServiceState.INITIALIZED:
            raise VisionLifecycleError(
                f"Cannot start VisionService in state {self._state.name}"
            )
    ...
    def stop(self) -> None:
        ...
        if self._in_transaction:
            raise VisionLifecycleError(
                "Cannot stop VisionService during an active transaction"
            )
        self._in_transaction = True
        failures: list[DeviceShutdownFailureRecord] = []
        try:
            # Clear frame stores and trackers across all sessions
            self.clear()
    ...
    def health(self) -> ServiceHealth:
        ...
        msg = f"Processed {self._processed_frames} frames across {len(self._session_stores)} active session(s), {self._dropped_frames} dropped"
    ...
    def _get_session_components(
        self, session_id: str
    ) -> tuple[VisionFrameStore, TemporalLandmarkTracker, VisionPipeline]:
    ...
    def ingest_frame(
        ...
        if self._state != ServiceState.STARTED:
            raise VisionLifecycleError(
                f"Cannot ingest frame in state {self._state.name}"
            )
        effective_session_id = (
            session_id or descriptor.session_id or self._default_session_id
        )
        if (
            session_id is not None
            and descriptor.session_id is not None
            and session_id != descriptor.session_id
        ):
            raise VisionSessionMismatchError(
                f"Envelope session_id {session_id!r} does not match payload session_id {descriptor.session_id!r}"
            )
        if descriptor.session_id != effective_session_id:
            stamped_desc = FrameDescriptor(
                frame_id=descriptor.frame_id,
                device_id=descriptor.device_id,
                physical_id=descriptor.physical_id,
                ...
                checksum_crc32=descriptor.checksum_crc32,
                epoch_id=descriptor.epoch_id,
                buffer_handle_id=descriptor.buffer_handle_id,
        ...
        if stamped_desc.epoch_id != self._epoch_id:
            raise VisionEpochMismatchError(
                f"Descriptor epoch {stamped_desc.epoch_id} does not match service epoch {self._epoch_id}"
            )
        # 2. Validate physical_id against DeviceRegistry
        self._verify_device_identity(stamped_desc)
        # 3. Sequence monotonicity verification
        seq_key = (
            effective_session_id,
            stamped_desc.device_id,
            stamped_desc.physical_id,
            stamped_desc.epoch_id,
        )
        if seq_key in self._session_sequences:
            ...
            if stamped_desc.sequence_number <= last_seq:
                ...
                raise VisionSequenceError(
                    f"Non-monotonic sequence number {stamped_desc.sequence_number} (last was {last_seq}) for session {seq_key}"
                )
        self._session_sequences[seq_key] = stamped_desc.sequence_number
        # 2. Frame execution within session isolate
        ...
        try:
            ...
            if self._dispatcher is not None:
                self._emit_event(
                    ...
                    {
                        "device_id": stamped_desc.device_id,
                        "frame_id": stamped_desc.frame_id,
                        "session_id": effective_session_id,
    ...
    def purge_session(self, session_id: str) -> None:
        """Atomically purge all transient frame stores, landmark trackers, and sequence history for a clinical session."""
        ...
        if sess_id and isinstance(sess_id, str) and SESSION_ID_REGEX.match(sess_id):
            if sess_id not in self._session_stores:
                ...
                pipeline = VisionPipeline(
                    tracker=tracker, secret_filter=self._secret_filter
                )
                self._session_stores[sess_id] = store
                self._session_trackers[sess_id] = tracker
    ...
    def handle_status_query(self, query_envelope: MessageEnvelope) -> MessageEnvelope:
        ...
        if sess_id and sess_id in self._session_stores:
            slot_states = [
                s.value for s in self._session_stores[sess_id].get_slot_states()
            ]
    ...
    def _verify_device_identity(self, descriptor: FrameDescriptor) -> None:
        ...
        try:
            ...
            raise VisionValidationError(
                f"Device {descriptor.device_id!r} is not registered in DeviceRegistry"
            )
