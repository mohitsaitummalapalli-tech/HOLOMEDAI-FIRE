...
class VisionService(IService):
    ...
    def initialize(self, context: RuntimeContext) -> None:
        ...
        if self._dispatcher is not None:
            ...
            self._dispatcher.subscribe_event("platform.session.activated", self.handle_session_activated_event, self.name)
    ...
    def _get_session_components(
        ...
        if session_id not in self._session_stores:
            ...
            raise VisionLifecycleError(
                ...
                f"Session {session_id} not activated"
    ...
    def ingest_frame(
        ...
        effective_session_id = session_id or descriptor.session_id or self._default_session_id



        if session_id is not None and descriptor.session_id is not None and session_id != descriptor.session_id:
    ...
    def purge_session(self, session_id: str) -> None:
        ...
        self._emit_event("vision.session.purged", {"session_id": session_id, "epoch_id": self._epoch_id})
    ...
    def handle_session_activated_event(self, event_envelope: MessageEnvelope) -> None:
        ...
        if sess_id and isinstance(sess_id, str) and SESSION_ID_REGEX.match(sess_id):
            ...
            if sess_id not in self._session_stores:
                ...
                pipeline = VisionPipeline(tracker=tracker, secret_filter=self._secret_filter)
